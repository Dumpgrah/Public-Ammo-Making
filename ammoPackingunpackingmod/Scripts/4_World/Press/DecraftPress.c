class TakeApartPress extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Take Apart";
		...
		//result1
		AddResult("PressBody");//add results here
		...
		AddResult("PressCasingholder");//add results here
		...
		m_ResultInheritsHealth[3] = -1;// (value) == -1 means do nothing; a (value) >= 0 means this result will inherit health from ingredient number (value);(value) == -2 means this result will inherit health from all ingredients averaged(result_health = combined_health_of_ingredients / number_of_ingredients)
