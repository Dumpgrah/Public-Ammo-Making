class Decraft545x39Tracer extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart 545x39Tracer ammo";
	...
	int OutcomeGunpowder = Math.Ceil(24 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(tipscount > 0 )
			{
				Tips = ItemBase.Cast(GetGame().CreateObject("Tips545x39", ResultGunpowder.GetPosition()));
			...
			if(casingcount > 0 )
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("Casing545x39", ResultGunpowder.GetPosition()));
