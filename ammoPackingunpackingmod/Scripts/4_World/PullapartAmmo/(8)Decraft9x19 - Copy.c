class Decraft380ACP extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart 380ACP ammo";
	...
	int OutcomeGunpowder = Math.Ceil(5 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(deformedtipscount > 0 )
			{
				...
				Tips = ItemBase.Cast(GetGame().CreateObject("DeFormedPistolTips", ResultGunpowder.GetPosition()));
				Tips.SetQuantity(deformedtipscount);
			}
			if(tipscount > 0 )
			{
				Tips = ItemBase.Cast(GetGame().CreateObject("Tips380ACP", ResultGunpowder.GetPosition()));
				Tips.SetQuantity(tipscount);
			}
			if(deformedcasingcount > 0)
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("DeFormedPistolCasing", ResultGunpowder.GetPosition()));
				Casing.SetQuantity(deformedcasingcount);
			}
			if(casingcount > 0 )
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("Casing380ACP", ResultGunpowder.GetPosition()));
