class Decraft357 extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart .357 Magnum ammo";
	...
	int OutcomeGunpowder = Math.Ceil(13 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(tipscount > 0 )
			{
				Tips = ItemBase.Cast(GetGame().CreateObject("Tips357", ResultGunpowder.GetPosition()));
			...
			if(casingcount > 0 )
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("Casing357", ResultGunpowder.GetPosition()));
