class Decraft12g extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart 12 gauge  ammo";
	...
	int OutcomeGunpowder = Math.Ceil(40 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(deformedtipscount > 0 )
			{
				...
				Tips = ItemBase.Cast(GetGame().CreateObject("TipsBuckshot", ResultGunpowder.GetPosition()));
			...
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("CasingBuckshot", ResultGunpowder.GetPosition()));
