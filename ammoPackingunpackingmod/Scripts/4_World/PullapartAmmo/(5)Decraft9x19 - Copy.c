class Decraft45ACP extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart 45 ACP ammo";
	...
	int OutcomeGunpowder = Math.Ceil(11 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(tipscount > 0 )
			{
				Tips = ItemBase.Cast(GetGame().CreateObject("Tips45ACP", ResultGunpowder.GetPosition()));
			...
			if(casingcount > 0 )
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("Casing45ACP", ResultGunpowder.GetPosition()));
