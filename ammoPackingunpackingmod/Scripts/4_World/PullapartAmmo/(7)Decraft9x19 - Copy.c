class Decraft308WinTracer extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart 308WinTracer ammo";
	...
	int OutcomeGunpowder = Math.Ceil(58 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(tipscount > 0 )
			{
				Tips = ItemBase.Cast(GetGame().CreateObject("Tips308Win", ResultGunpowder.GetPosition()));
			...
			if(casingcount > 0 )
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("Casing308Win", ResultGunpowder.GetPosition()));
