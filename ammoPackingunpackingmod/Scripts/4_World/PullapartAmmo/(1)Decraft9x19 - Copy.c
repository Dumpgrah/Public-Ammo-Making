...
{	
	...
	int OutcomeGunpowder = Math.Ceil(10 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(tipscount > 0 )
			{
				Tips = ItemBase.Cast(GetGame().CreateObject("Tips9X19", ResultGunpowder.GetPosition()));
			...
			if(casingcount > 0 )
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("Casing9X19", ResultGunpowder.GetPosition()));
