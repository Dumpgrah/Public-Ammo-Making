class Decraft556x45Tracer extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart 556x45Tracer ammo";
	...
	int OutcomeGunpowder = Math.Ceil(22 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(tipscount > 0 )
			{
				Tips = ItemBase.Cast(GetGame().CreateObject("Tips556x45", ResultGunpowder.GetPosition()));
			...
			if(casingcount > 0 )
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("Casing556x45", ResultGunpowder.GetPosition()));
