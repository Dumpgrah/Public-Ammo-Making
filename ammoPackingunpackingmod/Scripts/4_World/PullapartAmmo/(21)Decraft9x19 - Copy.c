class DecraftRubberSlugs extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Pull apart shotgun Rubber slug ammo";
	...
	int OutcomeGunpowder = Math.Ceil(33 * GetAmmoBuildingSettings().GetPercentageValueGunPowderLoss());
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		if(itemone > 1 )
		{
			...
			if(deformedtipscount > 0 )
			{
				...
				Tips = ItemBase.Cast(GetGame().CreateObject("TipsRubberSlug", ResultGunpowder.GetPosition()));
			...
			{
				Casing = ItemBase.Cast(GetGame().CreateObject("CasingSlug", ResultGunpowder.GetPosition()));
